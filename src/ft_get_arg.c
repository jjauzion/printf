...
/*   Updated: 2018/01/21 13:46:09 by jjauzion         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_var	ft_get_arg(va_list ap, t_spec *spec)
{
	...
	else if (ft_strchr("oOuUxX", spec->c_specifier))
	...
	else if (ft_strchr("c", spec->c_specifier) && ft_strchr(spec->l_modifier, 'l'))
		var.wc = va_arg(ap, wchar_t);
	else if (ft_strchr("c", spec->c_specifier))
		var.c = (char)va_arg(ap, int);
	else if (ft_strchr("S", spec->c_specifier))
		var.ws = va_arg(ap, wchar_t*);
	else if (ft_strchr("s", spec->c_specifier) && ft_strchr(spec->l_modifier, 'l'))
		var.ws = va_arg(ap, wchar_t*);
	else if (ft_strchr("s", spec->c_specifier))
